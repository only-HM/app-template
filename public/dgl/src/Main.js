...
window.addEventListener("load", function () {
     ...
     * 渲染模式
     */
    var stage = new annie.Stage("annieEngine", 750, 1334, 30, annie.StageScaleMode.SHOW_ALL, 0);
    //默认关闭自动旋转和自动resize
    window.stage = stage;
    if (annie.osType == "ios") {
    ...
    stage.addEventListener(annie.Event.ON_INIT_STAGE, function (e) {
        ...
        annie.loadScene("pf20240801", function (per) {
            //加载进度
            ...
            if (result.sceneId == result.sceneTotal) {
                stage.addChild(annie.getDisplay("pf20240801", "Pf20240801"));
            }
        });
    })
});